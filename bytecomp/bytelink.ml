...
let link_cmo file_name compunit =
...
let link_obj tolink = function
  | `Cmo (file_name, compunit) -> link_cmo file_name compunit :: tolink
  | `Cma (file_name, toc) -> link_cma file_name toc :: tolink
...
let link lib_resolver objfiles output_name =
  ...
  let lib_names = (* Names of fully linked libraries (including assumed) *)
    if !Clflags.link_everything then Some lib_seen else None
  in
